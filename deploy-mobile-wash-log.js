...
async function deploy() {
  try {
    ...
    const filesToUpload = [
      {
        local: path.join(__dirname, 'src/styles/wash-log.css'),
        remote: '/var/www/newrepo/src/styles/wash-log.css'
      },
      {
        local: path.join(__dirname, 'src/app/wash-log/components/ZorinWashLogClient.tsx'),
        remote: '/var/www/newrepo/src/app/wash-log/components/ZorinWashLogClient.tsx'
      }
    ];
    ...
    console.log('\n📱 Мобильная версия журнала моек:');
    console.log('  ✓ Карточный вид вместо таблицы');
    console.log('  ✓ Каждая мойка - отдельная карточка');
    console.log('  ✓ Эмодзи-иконки для категорий');
    console.log('  ✓ Крупная сумма (20px, зеленая)');
